...
class Server:
    def __init__(self, model_type, input_channels, flatten_size, dp_type, aggregation_method=None, attack_type=None, dataset=None):
        ...
        self.model.train() # TO DO: Why did I put thi?
        ...
        if self.logger is None and attack_type!="No_Attack":
            try:
                ...
                logging.getLogger("MalClientLogger").setLevel(logging.ERROR)  # Disable unnecessary logging
    ...
    def aggregate(self, local_params, aggregate_method, num_malicious_clients, client_data_sizes, client_grads):
        device = next(iter(local_params[0].values())).device  # Get device from first client

        client_grads = [
            {k: v.to(device) for k, v in client_dict.items()}
            for client_dict in client_grads
        ]
        for name in self.global_params:
            self.global_params[name] = self.global_params[name].to(device)

        if aggregate_method == "FedAvg":
            averaged_grads = fedAverage(client_grads, client_data_sizes)
            with torch.no_grad():
                for name in self.global_params.keys():
                    self.global_params[name] = self.global_params[name].float()
                    self.global_params[name] -= averaged_grads[name]

        if aggregate_method == "Krum":
            averaged_grads = krum(client_grads, num_malicious_clients, self.logger)
            with torch.no_grad():
                for name in self.global_params.keys():
                    if self.global_params[name].dtype != torch.float32:
                        self.global_params[name] = self.global_params[name].float()

                    self.global_params[name] -= averaged_grads[name].to(self.global_params[name].device)
        if aggregate_method == "MKrum":
            averaged_grads = mkrum(client_grads, num_malicious_clients, self.logger)
            with torch.no_grad():
                for name in self.global_params.keys():
                    if self.global_params[name].dtype != torch.float32:
                        self.global_params[name] = self.global_params[name].float()
        ...
        if aggregate_method == "TrimmedMean":
            averaged_grads = trimmed_mean(client_grads, num_malicious_clients)
            with torch.no_grad():
                for name in self.global_params.keys():
                    if self.global_params[name].dtype != torch.float32:
                        self.global_params[name] = self.global_params[name].float()
        ...
        if aggregate_method == "Median":
            averaged_grads = median_aggregation(client_grads)
            with torch.no_grad():
                for name in self.global_params.keys():
                    if self.global_params[name].dtype != torch.float32:
                        self.global_params[name] = self.global_params[name].float()
        ...
        if aggregate_method == "GeoMedian":
            averaged_grads = geometric_median_aggregation(client_grads)
            with torch.no_grad():
                for name in self.global_params.keys():
                    if self.global_params[name].dtype != torch.float32:
                        self.global_params[name] = self.global_params[name].float()
        ...
        if aggregate_method == "CenteredClipping":
            self.global_params = centered_clipping(client_grads, copy.deepcopy(self.global_params), 1)
        ...
        if aggregate_method == "Bulyan":
            averaged_grads = bulyan(client_grads, num_malicious_clients, self.logger)
            with torch.no_grad():
                for name in self.global_params.keys():
                    self.global_params[name] -= averaged_grads[name].to(self.global_params[name].device)
        ...
        for j in range(i + 1, num_clients):
            ...
            for key in local_params[i]:
                ...
                if param_i.dtype in [torch.float32, torch.float64, torch.float16]:
                    dist += ((param_i.to(device) - param_j.to(device)).norm()) ** 2
                else:
                    continue
...
def mkrum(local_params, num_malicious_clients, logger):
    ...
    csv_path = os.path.join(script_dir, 'mkrum_cifar10_non_iid_ipm_dp_scores.csv')
...
def median_aggregation(local_params):
    ...
    # Step 2: Compute the median for each parameter
    for key in local_params[0]:
        param_values = torch.stack([local_params[i][key] for i in range(num_clients)])
        
        aggregated_params[key] = torch.median(param_values, dim=0).values
...
# Centered Clipping method (tau is 1)
def centered_clipping(client_grads, global_params, tau=1):
    global_np = ordered_dict_to_numpy(global_params)

    clipped_gradients = []
    for client_grad in client_grads:
        grad_np = ordered_dict_to_numpy(client_grad)
        ...
        clipped_grad = grad_np * scale

        clipped_gradients.append(clipped_grad)

    # Compute average of clipped gradients
    average_clipped_gradient = np.mean(clipped_gradients, axis=0)

    # Apply the update
    updated_np = global_np - average_clipped_gradient

    updated_global_params = numpy_to_ordered_dict(updated_np, global_params)
...
#momentum_vector (numpy array): Previous momentum vector for temporal adjustment.
    ...
    log_file_path = 'cosine_logs/cifar_10_dp_ipm_clustering_cosine_similarity_log.csv'
...
def cos_defense(local_updates, global_params, logger):
    os.makedirs('cosine_logs', exist_ok=True)
    log_file_path = 'cosine_logs/mnist_dp_ipm_cosdefense_cosine_similarity_log.csv'
    ...
    # Log the indices of the chosen updates
    ...
    for key in global_params.keys():
        if key not in benign_updates[0]:  
            ...
            device = benign_updates[0][key].device  
            aggregated_update[key] = torch.stack(
                [update[key].to(device) for update in benign_updates if key in update]
            ).mean(dim=0)
...
def bulyan(local_updates, f, logger):
    ...
    # Store original indices of updates before filtering
...
#max_tau (float): Maximum threshold for clipping norm.
    os.makedirs('cosine_logs', exist_ok=True)
    log_file_path = 'cosine_logs/cifar_10_no_dp_ipm_clippedclustering_cosine_similarity_log.csv'
    ...
    flattened_inputs = [flatten_ordered_dict(OrderedDict(update)) if isinstance(update, dict) else update for update in local_updates]
    ...
    target_device = clipped_updates[0].device  
    clipped_updates = [update.to(target_device) for update in clipped_updates]
