...
def parse_args():
    ...
    parser.add_argument("--percentage_of_malicious", type=float, default=0, help="Percentage of malicious clients (e.g., 20 for 20%)")
    ...
    parser.add_argument("--dp_type", type=str, default="No_DP",
    ...
    parser.add_argument("--aggregate_method", type=str, default="FedAvg",
                        choices=["FedAvg", "Krum", "MKrum", "TrimmedMean", "Median", "GeoMedian",
                                 "CenteredClipping", "Clustering", "ClippedClustering", "CosDefense", "Bulyan"],
...
# Implemented aggregate methods: "FedAvg", "Krum", "MKrum", "TrimmedMean", "Median", GeoMedian, CenteredClipping, Clustering, ClippedClustering, FLVS, CosDefense, Bulyan ...
...
if __name__=="__main__":
    ...
    if not os.path.isdir('results'):
        os.mkdir('results')
    if not os.path.isdir('results/loss_accuracy_logs'):
        os.makedirs('results/loss_accuracy_logs') 
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        local_params, local_losses, max_epsilon = federated_training(clients)

        if DP_TYPE != "No_DP":
            if (max_epsilon >= EPSILON):
                print("break happens because of max_epsilon")
                break
        server.aggregate(local_params, aggregate_method = AGGREGATE_METHOD, num_malicious_clients=NUM_MALICIOUS_CLIENT, client_data_sizes=client_data_sizes)
        ...
        # Each client is updated with the aggregated state_dict.
