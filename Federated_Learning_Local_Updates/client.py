...
class Client:
        ...
        if self.data_loader: # If client has data to train
            if self.dp_type == "DP": # If DP is chosen
                if self.dataset_type == "CIFAR-10":
                    param, loss, local_epsilon = self.train_with_dp2()
                else:
                    param, loss, local_epsilon = self.train_with_dp()
                self.total_epsilon = local_epsilon
                print(f"Client {self.client_id} | Total ε: {self.total_epsilon:.4f}")
                if self.malicious and self.attack_type == "ModelP":
                    param = self.attack(param)
                return param, loss
            else: # If No_DP is chosen
                param, loss = self.train_without_dp()
                if self.malicious and self.attack_type == "ModelP":
                    param = self.attack(param)
                return param, loss
        else:
    ...
    # For Random Gaussian attack
    def attack(self, param):
        for key in param.keys():
            if param[key].dtype in [torch.float32, torch.float64, torch.float16]:
                param[key] += torch.randn(param[key].size(), device=param[key].device) * 20
        return param
    ...
    def train_without_dp(self):
        ...
        return self.model.state_dict(), train_loss / self.local_iteration
    ...
    def train_with_dp(self):
            ...
            return self.model.state_dict(), train_loss / self.local_iteration, privacy_epsilon
    ...
    def train_with_dp2(self):
            ...
            self.model.to('cpu') #40MB decrease
...
def federated_training(clients):
        local_params, local_losses = [], []

        for client in clients:
            param, loss = client.run()
        ...
        return copy.deepcopy(local_params), local_losses, max_epsilon
